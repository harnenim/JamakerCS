...
Line.prototype.render = function(index, last={ sync: 0, state: null }) {
	...
	if (!CM) this.renderHighlight(last);
	return this;
};
			...
			switch (e.key) {
				case "PageUp": {
					...
					if (hasFocus && !CM) {
						...
						if (!CM) editor.history.logIfCursorMoved();
					}
					break;
				}
				case "PageDown": {
	...
	setTimeout(() => {
		...
		setTimeout(() => {
			if (self.needToRender) {
				...
				// 렌더링 끝났으면 출력 새로고침
				if (!CM) self.input.dispatchEvent(new Event("scroll", { bubbles: true }));
			}
		}, 100);
	}, 1);
}
